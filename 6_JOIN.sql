-- 1. The first example shows the goal scored by a player with the last name 'Bender'. Modify it to show the matchid and player name for all goals scored by Germany. To identify German players, check for: teamid = 'GER'
...
SELECT matchid, mdate, COUNT(teamid) AS GermanGoals FROM goal
  JOIN game ON id = matchid
  WHERE teamid = 'GER'
  GROUP BY matchid, mdate;
